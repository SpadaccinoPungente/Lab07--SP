...
class Model:
    ...
    def isValida(self, parziale):
        citta_appena_inserita = parziale[-1].localita
        giorni_totali_citta = sum(1 for s in parziale if s.localita == citta_appena_inserita)
        if giorni_totali_citta > 6: return False

        if len(parziale) > 1 and parziale[-1].localita != parziale[-2].localita:
            # Se mi sto spostando, la città precedente deve aver accumulato almeno 3 giorni di fila.
            if len(parziale) < 4: return False
            citta_lasciata = parziale[-2].localita
            if parziale[-3].localita != citta_lasciata or parziale[-4].localita != citta_lasciata: return False
